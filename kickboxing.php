...
if (isset($_GET['search'])) {
  ...
  if (!$search) {
    header('location: ../kickboxing.php');
    exit();
  }
  ...
  <div class="row">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 p-4">
      ...
      <nav class="d-flex justify-content-center align-self-end <?php echo (isset($_GET['search']) && $pages == 0) || $page > $pages ? 'd-none' : '' ?>">
        <ul class="pagination">
          <li class="page-item <?php echo $page == 1 ? 'disabled' : '' ?>"><a class="page-link bg-dark text-white" href="kickboxing.php?page=<?php echo $previous ?>"><span aria-hidden="true">&laquo;</span></a></li>
          <?php for ($i = 1; $i <= $pages; $i++) : ?>
            <li class="page-item <?php echo $page == $i ? 'active' : '' ?>"><a class="page-link <?php echo $page == $i ? 'bg-warning border-warning' : 'text-dark' ?>" href="kickboxing.php?page=<?php echo isset($_GET['search']) ? 'search=' . $_GET['search'] . '&page=' . $i : 'page=' . $i ?>">
                <?php echo $i ?>
              </a></li>
          <?php endfor; ?>
          <li class="page-item <?php echo $page == $pages ? 'disabled' : '' ?>"><a class="page-link bg-dark text-white" href="kickboxing.php?page=<?php echo $next ?>"><span aria-hidden="true">&raquo;</span></a></li>
        </ul>
      </nav>
    </main>
  </div>
